...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/pipeline/download/:jobId", async (req, res) => {
    try {
      ...
      const resultsDir = path.join(process.cwd(), "pipeline", "LncRAnalyzer-summary");
...
async function executeDockerPipeline(jobId: string, data: any, files: Express.Multer.File[]) {
  try {
    ...
    dockerProcess.on("error", async (error) => {
      console.error(`[${jobId}] Docker execution error:`, error);
      await storage.updatePipelineJobStatus(jobId, "failed", `Docker execution error: ${error.message}`);
    });

  } catch (error) {
    console.error(`[${jobId}] Pipeline setup error:`, error);
    await storage.updatePipelineJobStatus(jobId, "failed", `Setup error: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
